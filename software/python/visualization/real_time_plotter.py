...
~Qt.WindowContextHelpButtonHint) 
        ...
        self.com_select.setFixedSize(100, 25)  # Set fixed size for COM port 
selection 
        ...
        self.amp_select.setFixedSize(100, 25)  # Set fixed size for 
Amplitude selection 
        ...
        self.ch_select.setFixedSize(100, 25)  # Set fixed size for Channel 
selection 
        ...
        self.sec_select.setFixedSize(100, 25)  # Set fixed size for Seconds 
selection 
    ...
    def get_config(self): 
        return self.com_select.currentText(), self.amp_select.value(), 
self.ch_select.value(), self.sec_select.value() 
...
class SerialThread(QThread): 
    ...
    def __init__(self, port, baudrate=115200, timeout=1): 
        super().__init__() 
        self.ser_port = serial.Serial(port, baudrate=baudrate, 
timeout=timeout) 
...
class SerialPlotter(pg.GraphicsLayoutWidget): 
    def __init__(self, channels, data_length, port, amplitude, 
baudrate=115200, timeout=1, bandpass_freq=[1, 45], 
                 notch_freq=50, sample_rate=250): 
        super().__init__() 
        self.channels = channels 
        self.num_channels = len(channels) 
        self.data_length = data_length 
        self.data = [np.zeros(data_length) for _ in 
                     range(self.num_channels)]  # Independent data storage 
for each channel 
        self.curves = [] 
        ...
        self.filters = [self.create_filters() for _ in 
range(self.num_channels)] 
 
    def create_filters(self): 
        b, a = iirfilter(5, Wn=self.bp, fs=self.fs, btype="bandpass", 
ftype="butter") 
        live_lfilter_bp = LiveLFilter(b, a) 
        q, p = iirfilter(5, [self.notch - 1.5, self.notch + 1.5], 
fs=self.fs, btype="bandstop", ftype="butter") 
        ...
        if data != '\r\n' and data != '' and data != '\n': 
            try: 
                ...
                for i in range(self.num_channels): 
                    filtered_value = 
self.filters[i][1](self.filters[i][0](data_values[i]))  # Apply filters 
...
if __name__ == '__main__': 
    ...
    if config_dialog.exec_() == QDialog.Accepted: 
        com_port, amplitude, no_of_channels, duration = 
config_dialog.get_config() 
        channels = create_channel_list(no_of_channels) 
        viewer = SerialPlotter(channels, duration * 250, com_port, 
amplitude) 
        viewer.show() 
        sys.exit(app.exec_())